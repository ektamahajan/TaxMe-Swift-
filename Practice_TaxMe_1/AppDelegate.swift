...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        // Setting up default local variable to hold value of previousCity and its Taxes for every time the App boots
        let defaults = NSUserDefaults.standardUserDefaults()
        let previousCity = ["PreviousCity" : "Cupertino"]
        let previousUseTax = ["PreviousUseTax" : "8.75"]
        let previousSalesTax = ["PreviousSalesTax" : "8.75"]
        defaults.registerDefaults(previousCity)
        defaults.registerDefaults(previousUseTax)
        defaults.registerDefaults(previousSalesTax)
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "com.ekta.mahajan.Practice_TaxMe_1" in the application's documents Application Support directory.
    ...
    lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource("Practice_TaxMe_1", withExtension: "momd")!
